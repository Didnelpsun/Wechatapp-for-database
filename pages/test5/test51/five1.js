...
Page({
  ...
  t1: function (e) {
    if  ((e.detail.y > (this.data.clientHeight / 4 - 15) && e.detail.y < (this.data.clientHeight / 4 + 15))|| (e.detail.y > (this.data.clientHeight / 2.5 - 15) && e.detail.y < (this.data.clientHeight / 2.5 + 15))) {
      this.setData({
        s1: 1,
      })
    } else
  ...
  check: function () {
    ...
    wx.showModal({
      title: '答案',
      content: '拖动：1订单：顾客、订单号、折扣规则、产品名 2支付：顾客、应付账款 3库存：零件号、仓库号、库存量',
      showCancel: false,
      confirmColor: "#7e0eb2",
      confirmText: "我知道了"
    })
